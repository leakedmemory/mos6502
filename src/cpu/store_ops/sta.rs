...
mod tests {
    ...
    fn sta_zero_page_test() {
        ...
        memory.borrow_mut().write(OPCODE_STA_ZPG, MEM_OFFSET);
        memory.borrow_mut().write(0x42, MEM_OFFSET + 1);
        memory.borrow_mut().write(OPCODE_STA_ZPG, MEM_OFFSET + 2);
        memory.borrow_mut().write(0x57, MEM_OFFSET + 3);
        memory.borrow_mut().write(OPCODE_STA_ZPG, MEM_OFFSET + 4);
        ...
        assert_eq!(memory.borrow().read(0x42), cpu.acc);
        ...
        cpu.acc = 0x00;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(0x57), cpu.acc);
        ...
        cpu.acc = 0x80;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(0x69), cpu.acc);
    ...
    fn sta_zero_page_x_test() {
        ...
        let memory = Memory::new();
        memory.borrow_mut().write(OPCODE_STA_ZPX, MEM_OFFSET);
        memory.borrow_mut().write(0x42, MEM_OFFSET + 1);
        memory.borrow_mut().write(OPCODE_STA_ZPX, MEM_OFFSET + 2);
        memory.borrow_mut().write(0x57, MEM_OFFSET + 3);
        memory.borrow_mut().write(OPCODE_STA_ZPX, MEM_OFFSET + 4);
        memory.borrow_mut().write(0x69, MEM_OFFSET + 5);
        ...
        cpu.acc = 0x32;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(X.wrapping_add(0x42).into()), cpu.acc);
        assert_eq!(cpu.pc - init_pc, BYTES);
        assert_eq!(cpu.cycles - init_cycles, CYCLES);
        assert_eq!(cpu.status, CPU_DEFAULT_STATUS);
        ...
        cpu.acc = 0x00;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(X.wrapping_add(0x57).into()), cpu.acc);
        assert_eq!(cpu.pc - pc_after_first_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_first_exec, CYCLES);
        assert_eq!(cpu.status, CPU_DEFAULT_STATUS);
        ...
        cpu.acc = 0x80;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(X.wrapping_add(0x69).into()), cpu.acc);
        assert_eq!(cpu.pc - pc_after_second_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_second_exec, CYCLES);
        assert_eq!(cpu.status, CPU_DEFAULT_STATUS);
    }
    ...
    fn sta_absolute_test() {
        ...
        cpu.acc = 0x32;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(0x8028), cpu.acc);
        assert_eq!(cpu.pc - init_pc, BYTES);
        assert_eq!(cpu.cycles - init_cycles, CYCLES);
        assert_eq!(cpu.status, CPU_DEFAULT_STATUS);
        ...
        cpu.acc = 0x00;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(0x2697), cpu.acc);
        assert_eq!(cpu.pc - pc_after_first_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_first_exec, CYCLES);
        assert_eq!(cpu.status, CPU_DEFAULT_STATUS);
        ...
        cpu.acc = 0x80;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(0x5570), cpu.acc);
        assert_eq!(cpu.pc - pc_after_second_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_second_exec, CYCLES);
        assert_eq!(cpu.status, CPU_DEFAULT_STATUS);
    }
    ...
    fn sta_absolute_x_test() {
        ...
        let memory = Memory::new();
        memory.borrow_mut().write(OPCODE_STA_ABX, MEM_OFFSET);
        memory.borrow_mut().write(0x28, MEM_OFFSET + 1);
        memory.borrow_mut().write(0x80, MEM_OFFSET + 2);
        memory.borrow_mut().write(OPCODE_STA_ABX, MEM_OFFSET + 3);
        memory.borrow_mut().write(0x97, MEM_OFFSET + 4);
        memory.borrow_mut().write(0x26, MEM_OFFSET + 5);
        memory.borrow_mut().write(OPCODE_STA_ABX, MEM_OFFSET + 6);
        memory.borrow_mut().write(0x70, MEM_OFFSET + 7);
        memory.borrow_mut().write(0x55, MEM_OFFSET + 8);
        ...
        cpu.execute_next_instruction();
        assert_eq!(
            memory.borrow().read(0x8028_u16.wrapping_add(X.into())),
            cpu.acc
        );
        assert_eq!(cpu.pc - init_pc, BYTES);
        assert_eq!(cpu.cycles - init_cycles, CYCLES);
        ...
        cpu.execute_next_instruction();
        assert_eq!(
            memory.borrow().read(0x2697_u16.wrapping_add(X.into())),
            cpu.acc
        );
        assert_eq!(cpu.pc - pc_after_first_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_first_exec, CYCLES);
        ...
        cpu.execute_next_instruction();
        assert_eq!(
            memory.borrow().read(0x5570_u16.wrapping_add(X.into())),
            cpu.acc
        );
        assert_eq!(cpu.pc - pc_after_second_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_second_exec, CYCLES);
    ...
    fn sta_absolute_y_test() {
        ...
        let memory = Memory::new();
        memory.borrow_mut().write(OPCODE_STA_ABY, MEM_OFFSET);
        memory.borrow_mut().write(0x28, MEM_OFFSET + 1);
        memory.borrow_mut().write(0x80, MEM_OFFSET + 2);
        memory.borrow_mut().write(OPCODE_STA_ABY, MEM_OFFSET + 3);
        memory.borrow_mut().write(0x97, MEM_OFFSET + 4);
        memory.borrow_mut().write(0x26, MEM_OFFSET + 5);
        memory.borrow_mut().write(OPCODE_STA_ABY, MEM_OFFSET + 6);
        memory.borrow_mut().write(0x70, MEM_OFFSET + 7);
        memory.borrow_mut().write(0x55, MEM_OFFSET + 8);
        ...
        cpu.execute_next_instruction();
        assert_eq!(
            memory.borrow().read(0x8028_u16.wrapping_add(Y.into())),
            cpu.acc
        );
        assert_eq!(cpu.pc - init_pc, BYTES);
        assert_eq!(cpu.cycles - init_cycles, CYCLES);
        ...
        cpu.execute_next_instruction();
        assert_eq!(
            memory.borrow().read(0x2697_u16.wrapping_add(Y.into())),
            cpu.acc
        );
        assert_eq!(cpu.pc - pc_after_first_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_first_exec, CYCLES);
        ...
        cpu.execute_next_instruction();
        assert_eq!(
            memory.borrow().read(0x5570_u16.wrapping_add(Y.into())),
            cpu.acc
        );
        assert_eq!(cpu.pc - pc_after_second_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_second_exec, CYCLES);
    ...
    fn sta_indirect_x_test() {
        ...
        memory.borrow_mut().write(OPCODE_STA_IDX, MEM_OFFSET);
        let zpg_addr = 0x08;
        memory.borrow_mut().write(zpg_addr, MEM_OFFSET + 1);
        let addr_l = X.wrapping_add(zpg_addr);
        let addr_h = X.wrapping_add(zpg_addr + 1);
        memory.borrow_mut().write(0x22, addr_l.into());
        memory.borrow_mut().write(0x32, addr_h.into());

        memory.borrow_mut().write(OPCODE_STA_IDX, MEM_OFFSET + 2);
        let zpg_addr = 0x57;
        memory.borrow_mut().write(zpg_addr, MEM_OFFSET + 3);
        let addr_l = X.wrapping_add(zpg_addr);
        let addr_h = X.wrapping_add(zpg_addr + 1);
        memory.borrow_mut().write(0x46, addr_l.into());
        memory.borrow_mut().write(0x83, addr_h.into());

        memory.borrow_mut().write(OPCODE_STA_IDX, MEM_OFFSET + 4);
        let zpg_addr = 0x69;
        memory.borrow_mut().write(zpg_addr, MEM_OFFSET + 5);
        let addr_l = X.wrapping_add(zpg_addr);
        let addr_h = X.wrapping_add(zpg_addr + 1);
        memory.borrow_mut().write(0x13, addr_l.into());
        memory.borrow_mut().write(0x77, addr_h.into());
        ...
        cpu.acc = 0x32;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(0x3222), cpu.acc);
        assert_eq!(cpu.pc - init_pc, BYTES);
        assert_eq!(cpu.cycles - init_cycles, CYCLES);
        assert_eq!(cpu.status, CPU_DEFAULT_STATUS);
        ...
        cpu.acc = 0x00;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(0x8346), cpu.acc);
        assert_eq!(cpu.pc - pc_after_first_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_first_exec, CYCLES);
        assert_eq!(cpu.status, CPU_DEFAULT_STATUS);
        ...
        cpu.acc = 0x80;
        cpu.execute_next_instruction();
        assert_eq!(memory.borrow().read(0x7713), cpu.acc);
        assert_eq!(cpu.pc - pc_after_second_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_second_exec, CYCLES);
        assert_eq!(cpu.status, CPU_DEFAULT_STATUS);
    }
    ...
    fn sta_indirect_y_test() {
        ...
        memory.borrow_mut().write(OPCODE_STA_IDY, MEM_OFFSET);
        let zpg_addr = 0x08;
        memory.borrow_mut().write(zpg_addr, MEM_OFFSET + 1);
        let addr_l = 0xDF;
        let addr_h = 0x32;
        memory.borrow_mut().write(addr_l, zpg_addr.into());
        memory
            .borrow_mut()
            .write(addr_h, zpg_addr.wrapping_add(1).into());

        memory.borrow_mut().write(OPCODE_STA_IDY, MEM_OFFSET + 2);
        let zpg_addr = 0x57;
        memory.borrow_mut().write(zpg_addr, MEM_OFFSET + 3);
        let addr_l = 0xCC;
        let addr_h = 0x83;
        ...
        memory.borrow_mut().write(OPCODE_STA_IDY, MEM_OFFSET + 4);
        let zpg_addr = 0x69;
        memory.borrow_mut().write(zpg_addr, MEM_OFFSET + 5);
        let addr_l = 0x86;
        let addr_h = 0x77;
        ...
        cpu.execute_next_instruction();
        assert_eq!(
            memory.borrow().read(0x32DF_u16.wrapping_add(Y.into())),
            cpu.acc
        );
        assert_eq!(cpu.pc - init_pc, BYTES);
        assert_eq!(cpu.cycles - init_cycles, CYCLES);
        ...
        cpu.execute_next_instruction();
        assert_eq!(
            memory.borrow().read(0x83CC_u16.wrapping_add(Y.into())),
            cpu.acc
        );
        assert_eq!(cpu.pc - pc_after_first_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_first_exec, CYCLES);
        ...
        cpu.execute_next_instruction();
        assert_eq!(
            memory.borrow().read(0x7786_u16.wrapping_add(Y.into())),
            cpu.acc
        );
        assert_eq!(cpu.pc - pc_after_second_exec, BYTES);
        assert_eq!(cpu.cycles - cycles_after_second_exec, CYCLES);
